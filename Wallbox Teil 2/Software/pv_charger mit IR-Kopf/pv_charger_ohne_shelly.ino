#define vers "ohne Shelly, Version: 14.11.2021"
...
#include <TimeLib.h> //wird nicht benötigt
#include <Time.h>
#define MY_NTP_SERVER "de.pool.ntp.org"         //NTP Zeit = Internetzeit   
#define MY_TZ "CET-1CEST,M3.5.0/02,M10.5.0/03"  //NTP Zeit = Internetzeit, Zeitzone und wann die Umstellung Winter-/Sommerzeit passiert  
...
#define relay_3ph 5//temporär auf 5 geändert 4 //Relay welches die Wallbox mit allen Phasen vom Netz trennt
#define relay_2ph 5 //eigentlich Pin4, der wird für den Zaehler auslesen benoetigt, Relay welches 2 Phasen dazu schaltet (von ein- auf dreiphasig)
...
unsigned long led_previousmicros = 0, ti_taster, ti_timer[4], shelly3em_lfd_nr=0, led_interval = 10000, tme;
//*********************************************************************************
void setup() {
  ...
  configTime(MY_TZ, MY_NTP_SERVER); //NTP Zeit = Internetzeit
...
void loop() {
  ...
  long stromzaehler;
  int v,e; //wird eigentlich nicht benoetigt, nur zum Testen
  stromzaehler=zaehler_LOGAREX(&v, &e);
  if(stromzaehler>=  -200000) {
    ...
    solar=stromzaehler-leistung; //es wird die PV Leistung ohne die aktuelle Leistung der Wallbox benötigt
    if(stromzaehler== -200000) solar==-30001;
    ...
    //shelly3em_status(); //Shelly3em abfragen, aktuelle Leistung als Integer in Watt
    //sprintf(telnet_buf,"Leistung VOR  Aktualisierung= %5d, Stromzaehler= %5d, Solar= %5d\n\r\0", leistung, stromzaehler, solar);  telnet_write(telnet_buf);
    //Serial.print(leistung); Serial.print(" leistung vor  Aktualisierung, Stromzaehler: ");Serial.println(stromzaehler);
    //solar=eingabe; //zum Testen ###################################################################################
    //Serial.print(leistung); Serial.print(" leistung nach Aktualisierung, Stromzaehler: ");Serial.println(stromzaehler);
    sprintf(telnet_buf,"Leistung NACH Aktualisierung= %5d, Stromzaehler= %5d, Solar= %5d ,\0", leistung, stromzaehler, stromzaehler-leistung);  telnet_write(telnet_buf);
...
void relay_schalten(boolean an) {
  ...
  if(wb!=1) {
    ...
    if(an==true) wallbox("fsp",0); //der go-echarger schaltet mit dem Befehl fsp zwischen 1- und 3-phasig um
    else wallbox("fsp",1);         //fsp=1 1-phasig und fsp=0 3-phasig
    delay(500);
  }
...
/*int shelly3em_status() {
  ...
  if (http.begin(client, adr)) {  // HTTP
    ...
    if (httpCode > 0) {   // httpCode will be negative on error
      ...
      //String payload = http.getString();Serial.println(payload); //so sthet es im Originalbeispiel
      //const size_t capacity = 2000; //ist vom JSON Assistenen eine Größenschätzung, hier mal willkürlich 300 angenommen
      ////const size_t capacity = JSON_OBJECT_SIZE(7) + 70; //ist vom JSON Assistenen eine Größenschätzung, hier mal willkürlich 300 angenommen
...
void sd(char* buf, unsigned long ut) {
  ...
  sprintf(dateiname_rem,"%04d%02d%02d.csv",tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday); //Statistikdateien die aelter als eine Woche sind werden geloescht
